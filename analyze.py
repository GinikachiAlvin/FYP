...
def create_pdf_report(df, confusion, filename):
    ...
    # Use the proper font path
    FONT_PATH = "C:\\Users\\lenovo\\pronunciation_project\\DejaVuSans.ttf"

    # Prepare the confusion matrix text
    formatted_matrix = format_confusion_matrix(confusion)
    ...
    for fname in os.listdir(folder):
        if fname.endswith(".wav"):
            ...
            try:
                ...
                if target_word_lower not in transcription.lower():
                    continue  # skip if word not found

                # Extract IPA for the target word from segments
                ipa_variants = [
                    seg["ipa"] for seg in segments
                    if seg["text"].lower() == target_word_lower and seg.get("ipa")
                ]
